...
public abstract class Board implements Serializable {
    ...
    public void rotate(){
        ...
        } catch (TetrisExcepcion e){
            switch (e.getMessage()) {
                case TetrisExcepcion.ROTATION_BOUNDS_DOWN:
                    while (outOfBounds(moving.getNextRotation(), moving.getXPos(), moving.getYPos())) {
                        movePieceUp();
                    }
                    moving.rotate();
                    break;
                case TetrisExcepcion.ROTATION_BOUNDS_LEFT:
                    while (outOfBounds(moving.getNextRotation(), moving.getXPos(), moving.getYPos())) {
                        movePieceRight();
                    }
                    moving.rotate();
                    break;
                case TetrisExcepcion.ROTATION_BOUNDS_RIGHT:
                    while (outOfBounds(moving.getNextRotation(), moving.getXPos(), moving.getYPos())) {
                        movePieceLeft();
                    }
                    moving.rotate();
                    break;
                case TetrisExcepcion.ROTATION_BOUNDS_UP:
                    while (outOfBounds(moving.getNextRotation(), moving.getXPos(), moving.getYPos())) {
                        movePieceDown();
                    }
                    moving.rotate();
    ...
    private boolean canMoveRight(int i, int j) {
        ...
        boolean canMove = true;
        try{
            if(pieceForm[i][j] && j + moving.getYPos() + 1 >= board[0].length) throw new TetrisExcepcion(TetrisExcepcion.CANT_GO_RIGHT);
    ...
    private boolean canMoveLeft(int i, int j) {
        ...
        boolean canMove = true;
        try{
            if(pieceForm[i][j] && j + moving.getYPos() - 1 < 0) throw new TetrisExcepcion(TetrisExcepcion.CANT_GO_LEFT);
            if(pieceForm[i][j] && (board[i + moving.getXPos()][j + moving.getYPos() - 1] || specialPiecesBoard[i + moving.getXPos()][j + moving.getYPos() - 1] != null)){
                canMove = false;
            }
        } catch (TetrisExcepcion e){
    ...
    private boolean canMoveDown(int i, int j){
        boolean[][] pieceForm = moving.getForm();
        try{
            if(pieceForm[i][j] && i + moving.getXPos() + 1 >= board.length) throw new TetrisExcepcion(TetrisExcepcion.CANT_GO_DOWN);
     ...
     * @throws TetrisExcepcion Lanza excepcion cuando la pieza se sale del tablero con un mensaje detallando en direccion se salio
     */
    private boolean canRotate() throws TetrisExcepcion {
        ...
        for (int i = 0; i < rotated.length; i++){
            for(int j = 0; j < rotated.length; j++){
                if(rotated[i][j] && i + x >= board.length) throw new TetrisExcepcion(TetrisExcepcion.ROTATION_BOUNDS_DOWN);
                if(rotated[i][j] && i + x < 0) throw new TetrisExcepcion(TetrisExcepcion.ROTATION_BOUNDS_UP);
                if(rotated[i][j] && j + y < 0) throw new TetrisExcepcion(TetrisExcepcion.ROTATION_BOUNDS_LEFT);
                if(rotated[i][j] && j + y >= board[0].length) throw new TetrisExcepcion(TetrisExcepcion.ROTATION_BOUNDS_RIGHT);
